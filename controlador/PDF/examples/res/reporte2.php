...
<?php
	...
	$sql="SELECT * FROM carrera,mencion,categoria WHERE id_carrera=cod_carrera AND id_mencion=codigo_mencion GROUP BY id_carrera,id_mencion,id_categoria ORDER BY nombre_carrera,nombre_mencion,nombre_categoria ASC ";
		...
		<page backleft="10mm" backtop="20mm" backright="10mm" backbottom="10mm" footer="page;heure;date">
			<div class="tabla">
				<page_header >
					...
					<h4 class="titulo" id="titu">LISTA DE CARRERAS, MENCIONES Y CATEGORÍAS.</h4>
				</page_header>';
				$select= pg_query("SELECT * FROM carrera,mencion,categoria WHERE id_carrera=cod_carrera AND id_mencion=codigo_mencion GROUP BY id_carrera,id_mencion,id_categoria ORDER BY nombre_carrera,nombre_mencion,nombre_categoria ASC LIMIT $lim OFFSET $off  ");
					...
					if($mostrar > 0)
					{
						while($fila=pg_fetch_array($select))
						{
							...
							echo"<tr>
								...
								<td style='width:21%;border-right:none; ";if($c!=$mostrar){echo"border-bottom:none;";} echo" '>
									$nombre_carrera
								</td>
								<td style='width:21%;border-right:none; ";if($c!=$mostrar){echo"border-bottom:none;";} echo" '>
									$tipo_carrera
								...
								<td style='width:21%;  ";if($c!=$mostrar){echo"border-bottom:none;";} echo" '>
									$nombre_categoria
								</td>";
							echo"</tr>";
						}
