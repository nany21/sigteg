...
<?php
	...
	$sql="SELECT * FROM categoria,mencion WHERE codigo_mencion=id_mencion AND status_categoria='Activo' AND codigo_categoria=codigo_categoria";
		...
		<page backleft="10mm" backtop="20mm" backright="10mm" backbottom="10mm" footer="page;heure;date">
			<div class="tabla">
				<page_header >
					...
					<h4 class="titulo" id="titu">DATOS DE CATEGORÍA.</h4>
				</page_header>';
				$select= pg_query("SELECT * FROM categoria,mencion WHERE codigo_mencion=id_mencion AND status_categoria='Activo' AND codigo_categoria=codigo_categoria LIMIT '1' ");
				...
				echo"<table border='1' style='margin-top:70px;margin-left:60px;'>";
					if($mostrar > 0)
					{
						while($fila=pg_fetch_array($select))
						{
							...
							$mencion=$fila['nombre_mencion'];
							$status=$fila['status_categoria'];
